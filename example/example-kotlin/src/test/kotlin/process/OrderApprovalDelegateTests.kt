...
class OrderApprovalDelegateTests {
  ...
  fun `should calculate order position for empty total`() {
    ...
    val execution =
      delegateExecutionFake()
        .withVariables(builder().set(ORDER_TOTAL, BigDecimal.valueOf(0.0)).build())
        .withVariablesLocal(
          builder()
  ...
  fun `should calculate order positions for existing total`() {
    ...
    val execution =
      delegateExecutionFake()
        .withVariables(builder().set(ORDER_TOTAL, BigDecimal.valueOf(12.99)).build())
        .withVariablesLocal(
          builder()
            .set(ORDER_POSITION, OrderPosition("position 1", BigDecimal.valueOf(10.10), 1))
            .build()
    ...
    // position 2
    OrderApproval()
      .calculateOrderPositions()
      .execute(
        execution.withVariablesLocal(
          builder()
            .set(ORDER_POSITION, OrderPosition("position 2", BigDecimal.valueOf(5.05), 2))
            .build()
        )
      )
