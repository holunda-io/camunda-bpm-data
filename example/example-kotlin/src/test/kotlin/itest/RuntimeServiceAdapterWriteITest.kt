...
class RuntimeServiceAdapterWriteITest : CamundaBpmDataITestBase() {
  ...
  fun `should write to task service adapter`() {
    ...
    whenever()
      .variable_are_modified_in_wait_state { runtimeService, executionId ->
        STRING_VAR.on(runtimeService, executionId).set("value")
        DATE_VAR.on(runtimeService, executionId).set(date)
        SHORT_VAR.on(runtimeService, executionId).set(11.toShort())
        INT_VAR.on(runtimeService, executionId).set(123)
        LONG_VAR.on(runtimeService, executionId).set(812L)
        DOUBLE_VAR.on(runtimeService, executionId).set(12.0)
        BOOLEAN_VAR.on(runtimeService, executionId).set(true)
        COMPLEX_VAR.on(runtimeService, executionId).set(complexValue)
        LIST_STRING_VAR.on(runtimeService, executionId).set(listOfStrings)
        SET_STRING_VAR.on(runtimeService, executionId).set(setOfStrings)
        MAP_STRING_DATE_VAR.on(runtimeService, executionId).set(map)
      }
      .and()
      .task_is_completed()
    then()
