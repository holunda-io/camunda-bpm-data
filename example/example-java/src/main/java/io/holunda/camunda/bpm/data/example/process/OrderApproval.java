...
public class OrderApproval {
  ...
  /**
   * Loads a primitive variable by id (string) and store a complex variable (order). Used in "Load Order" service task in BPMN ${loadOrder}
   *
   * @return Java delegate
   */
  @Bean
  public VariableAwareJavaDelegate loadOrder() {
    return execution -> {
      String orderId = execution.get(ORDER_ID);
   ...
   */
  @Bean
  public VariableAwareJavaDelegate calculateOrderPositions() {
    return execution -> {
      OrderPosition orderPosition = execution.get(ORDER_POSITION);
      BigDecimal oldTotal = execution.getOptional(ORDER_TOTAL).orElse(BigDecimal.ZERO);
      BigDecimal newTotal = oldTotal.add(orderPosition.getNetCost().multiply(BigDecimal.valueOf(orderPosition.getAmount())));
      execution.setLocal(ORDER_TOTAL, newTotal);
  ...
  /**
   * Checks that the variable "orderId" exists. Used as execution listener on start event in BPMN ${guardExecutionListener}
   *
  ...
  /**
   * Checks that the variable "orderApproved" exists. Used as task listener on complete of user task in BPMN ${taskExecutionListener}
   *
