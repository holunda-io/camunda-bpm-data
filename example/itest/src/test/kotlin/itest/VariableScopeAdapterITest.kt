...
class VariableScopeAdapterITest : CamundaBpmDataITestBase() {
  ...
  fun `should write to variable scope and read`() {
    ...
    whenever().process_is_started_with_variables(variables = createVariables())
    then()
      .variables_had_value(
  ...
  fun `should throw correct exceptions`() {
    ...
    given()
      .process_with_delegate_is_deployed(
        delegateExpression = "\${${DelegateConfiguration::readNonExisting.name}}"
      )

    whenever()
      .process_is_started_with_variables(
        variables = createVariableMapUntyped(),
        expectedException = VariableNotFoundException::class.java
      )
  }
  ...
  fun `should write to local variable scope and read`() {
    ...
    given()
      .process_with_modifying_delegate_is_deployed(
        modifyingDelegateExpression =
          "\${${DelegateConfiguration::writeVariablesToScopeAndLocal.name}}",
