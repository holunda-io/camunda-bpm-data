...
abstract class VariableGuardCondition<T>(
  ...
  fun evaluate(variableMap: VariableMap): List<GuardViolation<T>> {
    return evaluate(
      if (local) {
        VariableMapReader.of(variableFactory, variableMap).getLocalOptional()
      } else {
        VariableMapReader.of(variableFactory, variableMap).getOptional()
      }
    )
  }
  ...
  fun evaluate(variableScope: VariableScope): List<GuardViolation<T>> {
    return evaluate(
      if (local) {
        VariableScopeReader.of(variableFactory, variableScope).getLocalOptional()
      } else {
        VariableScopeReader.of(variableFactory, variableScope).getOptional()
      }
    )
  }
  ...
  fun evaluate(taskService: TaskService, taskId: String): List<GuardViolation<T>> {
    return evaluate(
      if (local) {
        TaskServiceVariableReader.of(variableFactory, taskService, taskId).getLocalOptional()
      } else {
        TaskServiceVariableReader.of(variableFactory, taskService, taskId).getOptional()
      }
    )
  }
  ...
  fun evaluate(runtimeService: RuntimeService, executionId: String): List<GuardViolation<T>> {
    return evaluate(
      if (local) {
        RuntimeServiceVariableReader.of(variableFactory, runtimeService, executionId).getLocalOptional()
      } else {
        RuntimeServiceVariableReader.of(variableFactory, runtimeService, executionId).getOptional()
      }
    )
  }
