...
import io.holunda.camunda.bpm.data.reader.LockedExternalTaskReader.Companion.of
...
class ReadAdapterLockedExternalTaskTest {
  ...
  fun `get UUID type from variableMap directly`() {
    ...
    assertThat(of(uuidVar, map).get()).isInstanceOf(UUID::class.java)
  }
  ...
  fun `get UUID type from ExternalTask`() {
    ...
    assertThat(of(uuidVar, task).get()).isInstanceOf(UUID::class.java)
  }
}
