...
import io.holunda.camunda.bpm.data.reader.VariableMapReader.Companion.of
...
internal class AntiCorruptionLayerTest {

  private val FOO = stringVariable("foo")
  private val BAZ = stringVariable("baz")

  private val TRANSIENT = customVariable("__transient", VariableMap::class.java)

  private val MY_ACL = CamundaBpmDataACL.guardTransformingLocalReplace(
    TRANSIENT.name,
    VariablesGuard(listOf(exists(FOO), BAZ.matches { it.length > 2 })),
    IdentityVariableMapTransformer
  )

  private val mapper = object : VariableMapTransformer {
    override fun transform(variableMap: VariableMap): VariableMap = builder()
      .set(FOO, of(FOO, variableMap).get())
      .set(BAZ, of(BAZ, variableMap).get().substring(1))
      .build()

  }

  private val MY_ACL2 = CamundaBpmDataACL.guardTransformingLocalReplace(
    TRANSIENT.name,
  ...
  fun `should wrap variables directly`() {
    ...
    assertThat(of(TRANSIENT, wrapped).get()).isEqualTo(vars)
  }
  ...
  fun `should check transform and wrap variables`() {
    ...
    assertThat(of(TRANSIENT, wrapped).get()).isEqualTo(mapper.transform(vars))
  }
