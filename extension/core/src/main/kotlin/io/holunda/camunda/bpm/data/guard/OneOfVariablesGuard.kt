...
class OneOfVariablesGuard(
  ...
  companion object {
     ...
     * Empty guard.
     */
    val EMPTY = OneOfVariablesGuard(listOf())
  }
   ...
   */
  fun fromExisting(condition: VariableGuardCondition<*>) = OneOfVariablesGuard(variableConditions.plus(condition))

  fun reduce(violations : List<List<GuardViolation<*>>>) : List<GuardViolation<*>> =
  ...
  fun evaluate(variableMap: VariableMap): List<GuardViolation<*>> =
    reduce(variableConditions.map { it.evaluate(variableMap) })
  ...
  fun evaluate(variableScope: VariableScope): List<GuardViolation<*>> =
    reduce(variableConditions.map { it.evaluate(variableScope) })
  ...
  fun evaluate(taskService: TaskService, taskId: String): List<GuardViolation<*>> =
    reduce(variableConditions.map { it.evaluate(taskService, taskId) })
  ...
  fun evaluate(runtimeService: RuntimeService, executionId: String): List<GuardViolation<*>> =
    reduce(variableConditions.map { it.evaluate(runtimeService, executionId) })
