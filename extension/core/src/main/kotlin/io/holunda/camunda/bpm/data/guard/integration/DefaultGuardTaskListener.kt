 ...
 */
class DefaultGuardTaskListener(val guard: VariablesGuard, val throwViolations: Boolean = true) :
  TaskListener {
  ...
  override fun notify(task: DelegateTask) {
    ...
    if (violations.isNotEmpty()) {
      val message =
        "${guard.getName() ?: "Guard"} violated in task '${task.name.removeNewLines()}' (taskId: '${task.id}')"
