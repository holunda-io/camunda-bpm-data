  ...
  public static <T, C extends Collection<T>> C readFromTypedValue(
    ...
    if (typedValue instanceof ObjectValueImpl) {
      ...
      try {
        ...
        if (values.isEmpty()) {
          ...
          if (memberClazz.isAssignableFrom(values.iterator().next().getClass())) {
            ...
            throw new WrongVariableTypeException(
                "Error reading "
                    + variableName
                    + ": Wrong member type detected, expected "
                    + memberClazz.getName()
        ...
        throw new WrongVariableTypeException(
            "Error reading " + variableName + ": Couldn't read value from " + json);
      }
    } else {
      throw new WrongVariableTypeException(
          "Error reading " + variableName + ": Couldn't read value from " + typedValue);
    }
  }
    ...
    if (typedValue instanceof ObjectValueImpl) {
      ...
      try {
        ...
        if (map.isEmpty()) {
          ...
          if (valueClazz.isAssignableFrom(entry.getValue().getClass())
              && keyClazz.isAssignableFrom(entry.getKey().getClass())) {
            return map;
          } else {
            throw new WrongVariableTypeException(
                "Error reading "
                    + variableName
                    + ": Wrong map type detected, expected key "
                    + keyClazz.getName()
                    + " and value "
                    + valueClazz.getName()
                    + ", but was not found in "
