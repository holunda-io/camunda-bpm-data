...
public class ProcessExecutionVariableReaderTest {
  ...
  public void mockExecution() {
    ...
    when(runtimeService.getVariableLocal(this.executionId, STRING.getName()))
  ...
  public void shouldDelegateGetOptional() {
    assertThat(CamundaBpmData.reader(runtimeService, executionId).getOptional(STRING))
        .hasValue(value);
    assertThat(
            CamundaBpmData.reader(runtimeService, executionId).getOptional(stringVariable("xxx")))
        .isEmpty();
  }

  @Test
  public void shouldDelegateGetLocalOptional() {
    assertThat(CamundaBpmData.reader(runtimeService, executionId).getLocalOptional(STRING))
        .hasValue(valueLocal);
    assertThat(
            CamundaBpmData.reader(runtimeService, executionId)
                .getLocalOptional(stringVariable("xxx")))
        .isEmpty();
  }

  @Test
  public void shouldDelegateGetLocal() {
    assertThat(CamundaBpmData.reader(runtimeService, executionId).getLocal(STRING))
        .isEqualTo(valueLocal);
  }
}
