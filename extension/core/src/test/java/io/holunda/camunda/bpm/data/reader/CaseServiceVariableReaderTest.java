...
public class CaseServiceVariableReaderTest {
  ...
  public void shouldDelegateGetOptional() {
    ...
    assertThat(CamundaBpmData.reader(caseService, taskId).getOptional(stringVariable("xxx")))
        .isEmpty();
  }

  @Test
  public void shouldDelegateGetLocalOptional() {
    assertThat(CamundaBpmData.reader(caseService, taskId).getLocalOptional(STRING))
        .hasValue(localValue);
    assertThat(CamundaBpmData.reader(caseService, taskId).getLocalOptional(stringVariable("xxx")))
        .isEmpty();
  }
