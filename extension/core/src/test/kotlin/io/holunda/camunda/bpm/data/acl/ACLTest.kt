...
@Deployment(
  resources =
    [
      "eventBasedSubprocess_no_transientMapping.bpmn",
      "eventBasedSubprocess_with_transientMapping.bpmn"]
)
class TransientVariableMappingListenerTest {

  @Suppress("RedundantVisibilityModifier") @get:Rule public val camunda = camunda()
  ...
  fun `NO ACL signal sub-process with variables sets variables on processInstance`() {
    ...
    // given
    val processInstance =
      camunda.runtimeService.startProcessInstanceByKey("eventBasedSubprocess_no_transientMapping")
  ...
  fun `ACL signal sub-process with variables does not set variables on processInstance`() {
    ...
    val processInstance =
      camunda.runtimeService.startProcessInstanceByKey("eventBasedSubprocess_with_transientMapping")
    BpmnAwareTests.assertThat(processInstance).isWaitingAt("event_wait_forever")
    // when
  ...
  fun `ACL signal sub-process with variables passes guard sets does not set variables on process instance`() {
    ...
    // Here the magic happens: all variables are wrapped in one transient variable
    val variables =
      ACL_GTLR.checkAndWrap(CamundaBpmData.builder().set(FOO, "bar").set(BAZ, 42L).build())
  ...
  companion object {
     ...
     *
     * The warp method will check the pre-conditions and create the transient representation. The
     * ACL is attached as an execution listener to the signal and verifies the pre-condition, and
     * then performs a transformation of variables.
     *
     * The conditions are:
     ...
     * - baz / 2 -> baz_internal
     */
    private val ACL_GTLR =
      CamundaBpmDataACL.guardTransformingLocalReplace(
        variableName = "transient_acl",
        variablesGuard = VariablesGuard(listOf(BAZ.matches { it > 40L }, FOO.exists())),
...
fun <T> VariableGuardCondition<T>.assertFor(
