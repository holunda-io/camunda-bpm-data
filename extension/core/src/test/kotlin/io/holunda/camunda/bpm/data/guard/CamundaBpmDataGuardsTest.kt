...
class CamundaBpmDataGuardsTest {
  ...
  fun `should construct exists condition`() {
    ...
    assertThat(condition.evaluate(Optional.empty()))
      .contains(
        GuardViolation(
          condition,
          Optional.empty(),
  ...
  fun `should construct notExists condition`() {
    ...
    assertThat(condition.evaluate(Optional.of("value")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("value"),
          "Expecting variable 'foo' not to be set, but it had a value of 'value'."
        )
      )
  }
    ...
    assertThat(condition.evaluate(Optional.of("value")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("value"),
          "Expecting local variable 'foo' not to be set, but it had a value of 'value'."
        )
      )
  }
  ...
  fun `should construct hasValue condition`() {
    ...
    assertThat(condition.evaluate(Optional.of("other")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("other"),
          "Expecting variable 'foo' to have value 'val', but it was 'other'."
        )
      )
  }
  ...
  fun `should construct hasValue local condition`() {
    ...
    assertThat(condition.toString())
      .isEqualTo("Value condition for local variable 'foo', value 'valLocal'")
    ...
    assertThat(condition.evaluate(Optional.of("other")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("other"),
          "Expecting local variable 'foo' to have value 'valLocal', but it was 'other'."
        )
      )
  }
  ...
  fun `should construct hasOneOfValues condition`() {
    ...
    assertThat(condition.toString())
      .isEqualTo("ValueIn condition for variable 'foo', values ['val1', 'val2']")
    ...
    assertThat(condition.evaluate(Optional.of("other")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("other"),
          "Expecting variable 'foo' to be one of ['val1', 'val2'], but it was 'other'."
        )
      )
  }
  ...
  fun `should construct hasOneOfValues local condition`() {
    ...
    assertThat(condition.toString())
      .isEqualTo("ValueIn condition for local variable 'foo', values ['valLocal1', 'valLocal2']")
    ...
    assertThat(condition.evaluate(Optional.of("other")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("other"),
          "Expecting local variable 'foo' to be one of ['valLocal1', 'valLocal2'], but it was 'other'."
        )
      )
  }
  ...
  fun `should construct matches condition`() {
    ...
    assertThat(condition.evaluate(Optional.of("mismatch")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("mismatch"),
          "Expecting variable 'foo' to match the condition, but its value 'mismatch' has not."
        )
      )
  }
    ...
    assertThat(condition.evaluate(Optional.of("mismatch")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("mismatch"),
          "Expecting local variable 'foo' to match the condition, but its value 'mismatch' has not."
        )
      )
  }
  ...
  fun `should construct matches condition with validation message supplier`() {
    val condition =
      CamundaBpmDataGuards.matches(FOO, { it == "special_val" }) {
        variableFactory,
        localLabel,
        option ->
  ...
  fun `should construct matches local condition with validation message supplier`() {
    val condition =
      CamundaBpmDataGuards.matchesLocal(FOO, { it == "special_val_local" }) {
        variableFactory,
        localLabel,
        option ->
    ...
    assertThat(condition.evaluate(Optional.of("mismatch")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("mismatch"),
          "Expecting variable 'foo' to match the regex '^special.*', but its value 'mismatch' has not."
        )
      )
  }
    ...
    assertThat(condition.evaluate(Optional.of("mismatch")))
      .contains(
        GuardViolation(
          condition,
          Optional.of("mismatch"),
          "Expecting local variable 'foo' to match the regex '^special.*', but its value 'mismatch' has not."
        )
      )
  }
  ...
  fun `kotlin should construct matches condition with Validation Message Supplier`() {
    val condition =
      FOO.matches({ variableFactory, localLabel, option ->
        "Expecting$localLabel variable '${variableFactory.name}' to be 'special_val', but its value '${option.get()}' has not."
      }) {
        it == "special_val"
      }
  ...
  fun `kotlin should construct matches local condition with Validation Message Supplier`() {
    val condition =
      FOO.matchesLocal({ variableFactory, localLabel, option ->
        "Expecting$localLabel variable '${variableFactory.name}' to be 'special_val_local', but its value '${option.get()}' has not."
      }) {
  ...
  fun `kotlin should construct matchesRegex local condition`() {
    ...
    assertThat(condition.evaluate(Optional.of("special_val"))).isEmpty()
    assertThat(condition.toString())
      .isEqualTo("MatchesRegex condition for local variable '${FOO.name}'")
  }
  ...
  fun `kotlin should construct matchesRegex E-Mail condition`() {
    ...
    assertThat(condition.local).isEqualTo(false)
    assertThat(condition.toString())
      .isEqualTo("MatchesRegex condition for variable '${EMAIL_VARIABLE.name}'")
    assertThat(condition.evaluate(Optional.of(EMAIL))).isEmpty()
  }
  ...
  fun `kotlin should construct matchesRegex E-Mail local condition`() {
    ...
    assertThat(condition.local).isEqualTo(true)
    assertThat(condition.toString())
      .isEqualTo("MatchesRegex condition for local variable '${EMAIL_VARIABLE.name}'")
    assertThat(condition.evaluate(Optional.of(EMAIL))).isEmpty()
  }
  ...
  fun `kotlin should construct matchesRegex UUID condition`() {
    ...
    assertThat(condition.local).isEqualTo(false)
    assertThat(condition.toString())
      .isEqualTo("MatchesRegex condition for variable '${UUID_VARIABLE.name}'")
    assertThat(condition.evaluate(Optional.of(MYUUID))).isEmpty()
  }
  ...
  fun `kotlin should construct matchesRegex UUID local condition`() {
    ...
    assertThat(condition.local).isEqualTo(true)
    assertThat(condition.toString())
      .isEqualTo("MatchesRegex condition for local variable '${UUID_VARIABLE.name}'")
    assertThat(condition.evaluate(Optional.of(MYUUID))).isEmpty()
  }
  ...
  fun `kotlin should construct validBean condition`() {
    ...
    assertThat(condition.local).isEqualTo(false)
    assertThat(condition.toString())
      .isEqualTo("ValidBean condition for variable '${UUID_VARIABLE.name}'")
    assertThat(condition.evaluate(Optional.of(MYUUID))).isEmpty()
  }
  ...
  fun `kotlin should construct validBean local condition`() {
    ...
    assertThat(condition.local).isEqualTo(true)
    assertThat(condition.toString())
      .isEqualTo("ValidBean condition for local variable '${UUID_VARIABLE.name}'")
    assertThat(condition.evaluate(Optional.of(MYUUID))).isEmpty()
  }
}
